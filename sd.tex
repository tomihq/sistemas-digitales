...
\[\begin{minipage}[b]{0.8\textwidth}
    ...
    \label{fig:clock_1}
\end{minipage}\]
Observamos que en esta imagen, hay 3 circuitos distintos que reciben entradas d y q pero además reciben un clock.
La diferencia entre la primera con las demás es que el clock está siempre escuchando cambios mientras que los demás solo escucha cuando llega al pico.
Podríamos decir que nosotros necesitamos que el clock solo suba, y cuando llega ese estado de suba aproveche para hacer cierta acción. Para hacer todo esto necesitamos un detector de pulso.
\begin{figure}[h]
    \begin{subfigure}{0.4\textwidth}
        \centering
...
Si mandamos input = 1, el primer momento queda 1 AND 1 y el AND es verdadero, pero luego de un momento queda 1 AND 0 y ahora el pico es 0. Si fuese 1 AND 1 todo el tiempo tendríamos un clock constante y no necesitamos eso.
TODO: Preguntar qué compuertas tienen delay
